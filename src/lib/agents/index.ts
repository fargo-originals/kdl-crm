...
import { buildColdOutreachPrompt } from './prompts/cold-outreach';
...
export async function runAgent(sessionId: string, userMessage: string) {
  ...
  if (!session || session.status === 'closed' || session.status === 'awaiting_human') return;
  ...
  // ── Sesión de campaña (outreach frío) ───────────────────────────────────────
  if (session.campaign_recipient_id && !session.lead_id) {
    ...
    systemPrompt = buildColdOutreachPrompt({
      businessName: vars.businessName ?? recipient.email,
      neighborhood: vars.neighborhood ?? 'Madrid',
      sector: campaign?.sector ?? 'negocio',
      hasWebsite: !!(vars.websiteUrl && vars.websiteUrl.trim() !== ''),
      rating: vars.rating,
      reviewCount: vars.reviewCount,
      assignedUserId: owner?.id ?? '',
    });

    replyEmail = recipient.email;
    replyName = vars.businessName ?? recipient.email;

  // ── Sesión de lead inbound ───────────────────────────────────────────────────
  } else {
    ...
    const locale = (lead?.locale ?? 'es') as 'es' | 'en';
    systemPrompt = buildQualifierPrompt(locale, {
    ...
    replyEmail = lead.email;
    replyName = lead.full_name;
  }
  ...
  const result = await generateText({
    ...
    system: systemPrompt,
    messages,
    tools: { proposeSlots, saveAnswer, requestHumanHandoff },
  ...
  if (channel === 'whatsapp') {
    const phone = session.lead_inquiries?.phone;
    if (phone) await sendWhatsAppMessage(phone, assistantReply);
  } else {
    await sendEmail(replyEmail, 'Re: KentoDevLab', buildAgentEmailHtml(assistantReply, replyName));
  }
